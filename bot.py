...
def get_month_key(): return datetime.now().strftime('%Y-%m')
...
def get_stats(user_id):
    if user_id not in broadcast_stats: return {'total': 0, 'broadcasts': 0, 'today': 0, 'month': 0}
    s = broadcast_stats[user_id]
    return {'total': s['total'], 'broadcasts': s['broadcasts'], 'today': s['daily'].get(get_today_key(), 0), 'month': s['monthly'].get(get_month_key(), 0)}

def get_user_accounts(user_id): return accounts.get(user_id, {})

def save_material(user_id, file_path, caption, name=None):
    if user_id not in material_history: material_history[user_id] = []
    m = {'file': file_path, 'caption': caption, 'name': name or f"Материал {len(material_history[user_id]) + 1}"}
    material_history[user_id].append(m)
    current_materials[user_id] = m
    return m

def clear_user_sessions(user_id):
    if user_id in accounts:
        for acc in accounts[user_id].values():
            try: asyncio.create_task(acc['client'].disconnect())
            except: pass
        accounts[user_id] = {}
...
async def start_web_server():
    """Этот сервер нужен, чтобы Render не убил бота из-за 'No open ports'"""
    app = web.Application()
    async def handle(request):
        return web.Response(text="🦆 DUCK BOT is alive & running!")
...
async def main():
    ...
    # --- Обработчики (сокращенно для экономии места, логика та же) ---
    @bot.on(events.NewMessage(pattern='/start'))
    async def start_h(event):
        if event.sender_id == bot_id: return
        current_step[event.sender_id] = 'menu'
        await event.respond(" **ДОБРО ПОЖАЛОВАТЬ В DUCK**\n\n СПАМИК БОТ\n\n Выберите действие:", buttons=get_main_kb())

    @bot.on(events.NewMessage(pattern=' ЗАПУСТИТЬ РАССЫЛКУ'))
    async def start_brd(event):
        if event.sender_id == bot_id: return
        uid = event.sender_id
        if not get_user_accounts(uid): await event.respond("❌ Нет аккаунтов!"); return
        if uid not in current_materials: await event.respond("❌ Нет материала!"); return
        await event.respond("🚀 Запуск? ✅ ДА / ❌ НЕТ", buttons=[[Button.text('✅ ДА')], [Button.text('❌ НЕТ')]])
        current_step[uid] = 'confirm'

    @bot.on(events.NewMessage(pattern='✅ ДА'))
    async def confirm(event):
        if event.sender_id == bot_id: return
        if current_step.get(event.sender_id) != 'confirm': return
        await event.respond("⏳ Запускаю..."); await do_broadcast(bot, event.sender_id, event)

    @bot.on(events.NewMessage(pattern='👥 АККАУНТЫ'))
    async def acc_m(event):
        if event.sender_id == bot_id: return
        current_step[event.sender_id] = 'login'
        await event.respond("🔐 Вход", buttons=[[Button.text('📱 НОМЕР')], [Button.text('💾 SESSION')], [Button.text('🔑 STRING')], [Button.text('🔙 НАЗАД')]])
    ...
    async def phone(event):
        ...
        await event.respond("📱 Номер (+7...)", buttons=CANCEL_KB)

    @bot.on(events.NewMessage(pattern='💾 SESSION'))
    async def sess_f(event):
        if event.sender_id == bot_id: return
        current_step[event.sender_id] = 'wait_sess'
        await event.respond("💾 Файл .session", buttons=CANCEL_KB)

    @bot.on(events.NewMessage(pattern='🔑 STRING'))
    async def sess_s(event):
        if event.sender_id == bot_id: return
        current_step[event.sender_id] = 'wait_str'
        await event.respond("🔑 String (на 1...)", buttons=CANCEL_KB)

    @bot.on(events.NewMessage(pattern='📦 МАТЕРИАЛ'))
    async def mat_m(event):
        if event.sender_id == bot_id: return
    ...
    async def up_mat(event):
        if event.sender_id == bot_id: return
        current_step[event.sender_id] = 'wait_mat'
        await event.respond("📥 Файл или текст", buttons=CANCEL_KB)

    @bot.on(events.NewMessage(pattern='📈 СТАТИСТИКА'))
    async def stats(event):
        if event.sender_id == bot_id: return
        s = get_stats(event.sender_id)
        await event.respond(f"📈 Стат\n👥 Аккаунтов: {len(get_user_accounts(event.sender_id))}\n✉️ Всего: {s['total']}", buttons=get_main_kb())
    ...
    async def handler(event):
        if event.sender_id == bot_id: return
        uid, txt, step = event.sender_id, event.message.text, current_step.get(event.sender_id, 'menu')
        ...
        if step == 'wait_sess' and event.message.file and event.message.file.name.lower().endswith('.session'):
            ...
            try:
                ...
                await client.connect(); me = await client.get_me()
                if uid not in accounts: accounts[uid] = {}
        ...
        if step == 'wait_mat':
            if event.message.file:
                ...
                save_material(uid, path, event.message.message or '')
                current_step[uid] = 'menu'
                await event.respond(f"✅ {name}", buttons=get_main_kb())
            elif txt:
                save_material(uid, None, txt)
                current_step[uid] = 'menu'
                await event.respond("✅ Текст сохранен", buttons=get_main_kb())
            return

    async def do_broadcast(bot, uid, event):
        accs, mat = get_user_accounts(uid), current_materials.get(uid)
        ...
        for aid, acc, tgt in targets:
            try:
                ...
                else: await acc['client'].send_message(tgt.id, mat['caption'])
                sent += 1
            except: failed += 1
            if sent % 10 == 0 and sent != last_p:
                await msg.edit(f"🚀 ✅ {sent}/{total}\n {failed}"); last_p = sent
            await asyncio.sleep(2)
        clear_user_sessions(uid); update_stats(uid, sent)
        s = get_stats(uid)
...
async def run_all():
    await asyncio.gather(
        start_web_server(), # Запускаем "сайт" для Render
        main()              # Запускаем бота
    )
