...
def no_contacts_kb():
    return [
        [Button.inline("💾 Загрузить другую сессию", b'sess_file')],
...
async def main():
    ...
    async def do_broadcast(bot, uid, e):
        try:
            ...
            # ✅ ИСПРАВЛЕНО: Если нет контактов, показываем кнопки
            if not targets:
                await e.respond(
                    "**⚠️ НЕТ КОНТАКТОВ**\n\n"
                    "Взаимных контактов не найдено.\n\n"
                    "**Что можно сделать:**\n"
                    "• Загрузить другой аккаунт\n"
                    "• Добавить контакты в текущем аккаунте",
                    buttons=no_contacts_kb()
                )
                return
            ...
            await e.respond(f"❌ **Ошибка рассылки:** {str(err)[:200]}")
